...
```
El ancho de pétalo de versicolor no cumple con una distribución normal, el cual es un requisito para sacar coeficiente de correlación; entendemos que no se debería hacer la prueba si no se normalizan los datos, pero ya que el propósito de este trabajo es practicar, continuaremos usándolos de cualquier manera.
...
## Regresión lineal
Cuando hicimos la representación con residuales, creamos el objeto *lineal* que contiene el modelo de la regresión lineal hecho por la función lm(), ahora, lo analizaremos:
```{r}
summary(lineal)
```
...
Para ver la relación entre los residuales y las variables, graficaremos en torno a 0 con variabilidad constante a lo largo de X.
```{r}
...
chisq.test(matriz)
```
Si la prueba no se rechaza, indica que no depende una variable de la otra. Si la prueba sí se rechaza, indica que sí depende una variable de la otra.
