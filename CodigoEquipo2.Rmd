...
Todos %>% group_by(gene_symbol, dsm_iv_clinical_diagnosis) %>%
  ggplot(aes(dsm_iv_clinical_diagnosis,value,fill = dsm_iv_clinical_diagnosis)) +
  geom_boxplot(outlier.shape = NA, size = 0.71, fatten = 1) +
  stat_summary(fun=mean, geom = "point", shape = 16, size = 2.5, color = "red") +
  scale_fill_manual(values = c(colorspace::lighten("#D184C9", 0.5),
                               colorspace::lighten("#6684C9", 0.5))) +
  labs(x = "Diagnóstico DSM-IV", y = "ARN normalizado (FPKM)",
       fill = "Diagnóstico DSM-IV") +
  ggtitle("Niveles de ARN en sustancia blanca de lóbulo parietal derecho") +
  ylim(0,7500) + facet_grid(~gene_symbol) +
  ...
  #ggsignif::geom_signif(comparisons = list(c("Sin Demencia", "Demencia tipo AD")),
  #test = "t.test", map_signif_level = TRUE)
  ggpubr::stat_compare_means(method = "t.test", label = "p.signif",
                             hide.ns = TRUE, comparisons = list(c("Sin Demencia", "Demencia tipo AD")), size = 6)
#con hide.ns se quita el NS pero queda el bracket.
#Guardando previamente la localización de donde se quiere poner el bracket, no se genera correctamente el plot.
