...
stat.test <- Todos %>% 
  ...
  t_test(value ~ dsm_iv_clinical_diagnosis) %>%
  add_significance() %>%
  filter(p.signif != "ns")

Todos %>% group_by(gene_symbol, dsm_iv_clinical_diagnosis) %>%
  ggplot(aes(dsm_iv_clinical_diagnosis,value)) +
  geom_boxplot(aes(fill = dsm_iv_clinical_diagnosis), outlier.shape = NA, size = 0.71, fatten = 1) +
  stat_pvalue_manual(stat.test, label = "p.signif", y.position = 7000, size = 7) +
