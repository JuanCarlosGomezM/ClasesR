...
####Guardamos la informacion pertinente de los genes y de las muestras
```{r}
...
#####Unimos con la informacion de las muestras
...
#####Unimos con la información relevante de los donadores
Nos aseguramos de que correspondan usando la etiqueta *donor_id*.
(Nota: Ejecutar este chunk puede ser tardado)
```{r}
rna.measure.genes <- merge(donor.information[, .(donor_id = as.factor(donor_id),
    ever_tbi_w_loc, braak, dsm_iv_clinical_diagnosis )], rna.measure.genes,
    by = "donor_id")
```
---
...
Cabe recordar que el nivel de significancia, alfa, es la probabilidad de
...
en el eje de las ordenadas está la dispersión real de la muestra. 
...
```
El test no encontró diferencias significativas en las varianzas de MBP
en los dos grupos (p>alfa).

```{r}
bartlett.test(list(grupo_AD$value.PLP1,grupo_control$value.PLP1))
```
El test si encontró diferencias significativas en las varianzas de PLP1
...
###Pruebas t para comparar 2 grupos independientes
...
```
**Hay diferencia** (*p-value = 0.02246).
Diferencia media = 2047.141 - 3202.342 = -1155.201
Es negativo porque hay menos MBP en grupo con AD que en grupo control
...
###Gráfico de barras
Hacemos un gráfico de barras de los niveles promedio,
...
con asterisco indicando en cuál variable se
encuentra la diferencia (no confundir con el punto del valor atípico).
```{r}
Todos %>% group_by(gene_symbol, dsm_iv_clinical_diagnosis) %>%
