...
(Los 2 números al final de cada código de colores
...
Todos %>% group_by(gene_symbol, dsm_iv_clinical_diagnosis) %>%
  ...
  scale_fill_manual(values = c("#D184C995", "#6684C995")) +
  labs(x = "Diagnóstico DSM-IV", y = "ARN normalizado (FPKM)", fill = "Diagnóstico DSM-IV") +
  ggtitle("Niveles de ARN en sustancia blanca de lóbulo parietal derecho") +
  ylim(0,7500) + facet_grid(~gene_symbol) +
  theme(panel.spacing.x = unit(2, "lines"),legend.position = "none",
        plot.title = element_text(hjust = 0.5)) +
  ggpubr::stat_compare_means(method = "t.test", label = "p.signif",
                             hide.ns = TRUE, label.y = 5500, label.x.npc = 0.99)
